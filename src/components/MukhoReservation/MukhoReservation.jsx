...
const MukhoReservation = () => {
  ...
  useEffect(() => {
    if (picked.length > 0) {
      ...
      const pricePerWeek = MUKHO_PRICE.RENT_PER_WEEK + MUKHO_PRICE.MANAGEMENT_PER_WEEK;
      const calculatedUsagePrice = pricePerWeek * calculatedWeeks + MUKHO_PRICE.CLEANING_FEE;
      const calculatedTotal = calculatedUsagePrice + MUKHO_PRICE.DEPOSIT;
    ...
    <div className="common-reservation">
      ...
      <section className="price-total-section">
        ...
        <div className="price-detail">
          <p style={{fontSize: '15px'}}><b>임대료:</b> {MUKHO_PRICE.RENT_PER_WEEK.toLocaleString()}원 x {weeks}주 = {(MUKHO_PRICE.RENT_PER_WEEK * weeks).toLocaleString()}원</p>
          <p style={{fontSize: '15px'}}><b>관리비:</b> {MUKHO_PRICE.MANAGEMENT_PER_WEEK.toLocaleString()}원 x {weeks}주 = {(MUKHO_PRICE.MANAGEMENT_PER_WEEK * weeks).toLocaleString()}원</p>
          <p style={{fontSize: '15px'}}><b>청소비:</b> {MUKHO_PRICE.CLEANING_FEE.toLocaleString()}원</p>
          <div style={{marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #eee'}}>
            <p style={{fontSize: '15px', fontWeight: '600', marginBottom: '8px'}}><b>실제 이용요금:</b> {usagePrice.toLocaleString()}원</p>
            <p style={{fontSize: '15px', color: '#666', marginBottom: '8px'}}><b>보증금:</b> {MUKHO_PRICE.DEPOSIT.toLocaleString()}원</p>
